...
def _load_model(path: str) -> tuple[Any, dict[str, list[str]]]:
...
def infer_over_probs(*, engine: Engine, model_path: str, snapshot_id: str) -> InferenceResult:
    """
    Run the baseline logistic regression model for a snapshot and return P(over) per row.
    """
    ...
    # Mirror training-time coercions (app.ml.train._prepare_features).
    for col in cat_cols:
    ...
    X = frame[cat_cols + num_cols].copy()
    # Hardening: ensure finite numerics (sklearn can emit noisy matmul warnings otherwise).
