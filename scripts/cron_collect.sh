...
if [ "${#modal_cmd[@]}" -eq 0 ]; then
  echo "cron_collect error: modal CLI not found (checked .venv, MODAL_BIN, PATH, and common user install locations)" | tee -a "$LOG_FILE" | tee "$tmp_log"
  echo "cron_collect error: PATH=$PATH" | tee -a "$LOG_FILE" | tee -a "$tmp_log"
...
echo "cron_collect info: using modal command: ${modal_cmd[*]}" >> "$LOG_FILE"

: > "$tmp_log"

if ! "${modal_cmd[@]}" run "$PROJECT_ROOT/modal_app.py::collect_now" \
